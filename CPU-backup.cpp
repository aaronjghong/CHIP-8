...
// Creating "Hardware" | Note, a "stack pointer" is not necessary due to use of std::vector and its in-built method for h_STACK
uint8_t h_RAM[0xFFF]; // 0xFFF bytes (4096) of memory
...
void playSound(){}; // Does nothing right now

// Fetch
uint8_t readNextOpcode(){
...
void OPC_8XY4(uint16_t opc){ // Double check overflow/underflow without using & to get lower 8 bits
    uint8_t VX = (opc & 0x0F00) >> 8;
    uint8_t VY = (opc & 0x00F0) >> 4;
...
void OPC_8XY5(uint16_t opc){ // Double check overflow/underflow without using & to get lower 8 bits
    uint8_t VX = (opc & 0x0F00) >> 8;
    uint8_t VY = (opc & 0x00F0) >> 4;
...
void OPC_8XY6(uint16_t opc){ // Has two different "modes"
    uint8_t VX = (opc & 0x0F00) >> 8;
    //uint8_t VY = (opc & 0x00F0) >> 4;
...
void OPC_8XYE(uint16_t opc){ // Has two different "modes"
    uint8_t VX = (opc & 0x0F00) >> 8;
    //uint8_t VY = (opc & 0x00F0) >> 4;
    //h_REGISTERS[VX] = h_REGISTERS[VY]; // Either we have this step or ignore this step (depending on CHIP-8 version)
